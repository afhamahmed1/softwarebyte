                ...
                <div class="card-body" style="overflow-y:auto">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <?php
                            $items = getAll('services');
                            if (mysqli_num_rows($items) > 0) {
                                foreach ($items as $item) {
                                    $category_name = mysqli_fetch_assoc(section_name("services", "categories", "category_id", $item["category_id"]));
                                    $tag_id = mysqli_fetch_all(getByColumn('service_tags', 'section_id', $item['id']));
                            ?>
                                    <tr>
                                        <?php
                                        if ($item['image'] != "") {
                                        ...
                                        <td class="d-flex gap-2">
                                            <a href="edit_service.php?id=<?= $item['id'] ?>&section=service" class="btn btn-primary">Edit</a>
